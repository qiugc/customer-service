...
export class DispatchService {
  ...
  public registerHandlers(socket: socketIo.Socket): void {
    ...
    socket.on('messageService-getMessages', async (data, callback) => {
      console.log('messageService-getMessages', data,callback);
      const { ctx, msgs } = data;
      this.log.info('收到“'+msgs[0].sender+'”消息：'+msgs[0].content);
  ...
  > {
    ...
    tasks.forEach((task) => {
      const t = {
        task_id: String(task.dataValues.id),
        app_id: task.dataValues.app_id,
        env_id: new Date().getTime(),
      };
      ts.push(t)
    });
      console.log('updateTasks', ts);
